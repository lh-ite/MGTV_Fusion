...
%     Y  = (mm1.*M1) + (mm2.*M2);
    ...
    case 7,
        filter_size = 3;
        mean_A = imfilter(M1,ones(filter_size));
        mean_B = imfilter(M2,ones(filter_size));
        median_A = medfilt2(M1,[3,3]);
        median_B = medfilt2(M2,[3,3]);
        s1 = norm(mean_A-median_A,"fro");
        s2 = norm(mean_B-median_B,"fro");
        w1 = s1/(s1+s2);
        w2 = s2/(s1+s2);
        Y = w1*M1+w2*M2;
    case 8,
        en_1 = compute_entropy(M1);
        en_2 = compute_entropy(M2);
        w1 = en_1/(en_1+en_2);
        w2 = en_1/(en_1+en_2);
        Y = w1*M1 +w2*M2;
    case 9,
        S1 = silence_extract(M1);
        S2 = silence_extract(M2);
        O1 = normalize(S1);
        O2 = normalize(S2);
        w1 = O1/(O1+O2);
        w2 = O2/(O1+O2);
